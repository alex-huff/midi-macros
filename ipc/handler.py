...
def handleProfileMessage(message, position, midiMacros):
    if (len(message) < position + 2):
        return failResponse('not enough arguments, profile name and message type required')
...
def handleVirtualSustainMessage(message, position, midiListener):
    if (len(message) != position + 1):
        return failResponse('profile virtual-sustain message takes exactly one argument')
    action = message[position]
    if (action == TOGGLE):
        enabled = midiListener.toggleVirtualPedalDown()
        return successResponse(f'virtual-sustain is now {"enabled" if enabled else "disabled"}')
    elif (action == ENABLE):
